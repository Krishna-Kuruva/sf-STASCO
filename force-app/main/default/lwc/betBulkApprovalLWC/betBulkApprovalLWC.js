...
import getBulkApprovalRecords from '@salesforce/apex/BetExportApprovalDataController.getBulkApprovalRecords';
import updateApproved from '@salesforce/apex/BetBulkUploadLWCController.updateApproved';
const columns = [{
        label: 'BDD Form Name',
        fieldName: 'LinkName',
        type: 'url',
...
export default class BetBulkApprovalLWC extends LightningElement {
    @track value;
    @track error;
    @track data;
    ...
    @wire(getBulkApprovalRecords, {searchKey: '$searchKey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'}) formList(result) {
    ...
    getSelectedRec() {
        ...
        if(selectedRecords.length > 0){
            ...
            updateApproved({ selectedFormList: selectedRecords })
            .then(result => {
